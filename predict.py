...
def step_forward(t):
    ...
    force = tools.update_force_prediction(
        p_a, v_a, tools.k, tools.mass, tools.g, tools.density, dens_a, v_current_a, tools.height)
    ...
    direction = (p_a-p_b).astype(float)  # 搜救潜艇前进方向
    direction /= np.linalg.norm(direction)
    v = tools.v_s * direction
    tools.pos_s = tools.update_position(tools.pos_s, v)

    # 如果真实位置在探测范围内，说明探测成功
    if tools.distance_point_to_segment(tools.pos, p_b, tools.pos_s) <= tools.R:
        tools.finished = True
    else:
